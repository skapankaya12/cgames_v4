...
import type { CVAnalysisResult, CVData } from '@cgames/types/CVTypes';

// Re-export for backwards compatibility
export type { CVData, CVAnalysisResult } from '@cgames/types/CVTypes';

// Configure PDF.js worker for version 3.x compatibility
if (typeof window !== 'undefined') {
  // In browser environment, use CDN worker for v3.x
  try {
    pdfjsLib.GlobalWorkerOptions.workerSrc = `https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js`;
    console.log('✅ PDF.js worker configured with CDN v3.x');
  } catch (error) {
    // Fallback - disable worker (slower but more compatible)
    console.warn('⚠️ Worker setup failed, using fallback mode:', error);
  ...
  async extractCVData(file: File, testScores?: any[]): Promise<CVData> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`CV yükleme hatası: ${error.message}`);
      } else {
