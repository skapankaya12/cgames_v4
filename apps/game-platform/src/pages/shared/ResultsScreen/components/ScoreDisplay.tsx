...
export const ScoreDisplay: React.FC<ScoreDisplayProps> = ({ scores, onShowHelp }) => {
    ...
    return (
      <div className="score-display empty-state">
        <div className="empty-state-content">
          ...
          <h3>{t('results.competencies.noDataTitle')}</h3>
          <p>{t('results.competencies.noDataDescription')}</p>
        </div>
      </div>
    );
  }
  ...
  const getCompetencyDescription = (competencyName: string) => {
    // Try to get the description from translations first
    const translationKey = `results.competencyDescriptions.${competencyName}`;
    const translatedDescription = t(translationKey);
    
    // If translation exists, use it; otherwise fallback to a generic message
    if (translatedDescription !== translationKey) {
      return translatedDescription;
    }
    
    // Fallback message for missing descriptions
    return `${competencyName}, profesyonel yaşamda önemli olan ve sürekli geliştirilmesi gereken temel yetkinliklerden biridir. Bu alan, iş performansını ve kariyer gelişimini doğrudan etkileyen kritik beceriler içermektedir.`;
  };

  return (
    <div className="score-display">
      <div className="section-header">
        <h2>{t('results.competencies.title')}</h2>
        <button className="section-help-button" onClick={() => onShowHelp('competencies')}>
          <Icons.Lightbulb size={20} />
          {t('results.competencies.help')}
        </button>
      </div>
      ...
      <div className="scores-grid">
        {scores.map((score, index) => {
          ...
          const getPerformanceLevel = (percentage: number) => {
            if (percentage >= 80) return { label: t('results.performanceLevels.excellent'), color: '#10b981' };
            if (percentage >= 60) return { label: t('results.performanceLevels.good'), color: '#708238' };
            if (percentage >= 40) return { label: t('results.performanceLevels.average'), color: '#f59e0b' };
            return { label: t('results.performanceLevels.needsDevelopment'), color: '#ef4444' };
          };
              ...
              <div className="score-details">
                <div className="score-breakdown">
                  <span className="score-raw">
                    {score.score} / {score.maxScore} {t('results.competencies.points')}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="score-summary">
        <div className="summary-card best-score">
          ...
          <div className="summary-content">
            <h4>{t('results.competencies.bestPerformance')}</h4>
        ...
        <div className="summary-card needs-improvement">
          ...
          <div className="summary-content">
            <h4>{t('results.competencies.developmentArea')}</h4>
        ...
        <div className="summary-card average-score">
          ...
          <div className="summary-content">
            <h4>{t('results.competencies.averagePerformance')}</h4>
            <p>{scores.length} {t('results.competencies.competencyAreas')}</p>
            <span className="summary-score">
